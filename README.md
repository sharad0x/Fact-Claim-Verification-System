...
* **Omni-Input Processing:** Seamlessly accepts raw text, URLs, Documents (PDF/DOCX), Images (Photographs, Memes, Chat Screenshots), and **Audio (.mp3, .wav, .m4a)**.
...
* **Native Audio Deepfake Analysis:** Utilizes Google's Gemini 2.5 Flash Multimodal LLM to directly ingest raw audio streams, analyzing acoustic footprints for synthetic voice cloning (e.g., ElevenLabs), unnatural breathing cadences, and robotic phasing.
...
* **Production-Ready UI:** Features a Live Agent Terminal with Server-Sent Events (SSE) streaming, Dark/Light mode toggles, dynamic stage mapping, and interactive pipeline breakdowns.
...
* **Resilient API Handling:** Built with "Graceful Degradation" and "Safety Catchers" to automatically handle API timeouts, massive payload drops, formatting hallucinations, and strict privacy guardrails without crashing the user experience.
...
1.  **Phase 1: Pre-Processing & Omni-Parsing**
    ...
    * **Audio files are routed through `LlamaParse` for accurate Automatic Speech Recognition (ASR) and transcription.**
2.  **Phase 2: Deepfake & Media Analysis (Parallel Routing)**
    * **Images:** Routed through the 3-Stage Ensemble. If the Vision model detects a "screenshot" (like a WhatsApp chat or News Graphic), it intelligently bypasses the mathematical forensics (to prevent false alarms on UI text) and performs OCR to pass the text down to the fact-checker.
    * **Audio:** The raw acoustic stream is isolated and sent to the Multimodal LLM to generate an AI-voice probability score, operating entirely independently of the transcribed text.
3.  **Phase 3: AI Text Forensics**
    * Extracted text (from documents, OCR, or audio transcription) is analyzed for synthetic generation markers.
...
### Prerequisites
* Python 3.10+
* API Keys for NVIDIA NIM, Google Gemini, LlamaCloud, and Tavily.
...
- `GEMINI_API_KEY`: Required for the native audio deepfake analysis and multimodal acoustic reasoning (Gemini 2.5 Flash). Get it from [Google AI Studio](https://aistudio.google.com/).
